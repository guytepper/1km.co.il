...
function createUserFollowRequestId(protestId, fcmToken) {
  return `${protestId}${fcmToken}`;
}

export async function setUserFollow({ protestId, fcmToken }) {
  if (!protestId || !fcmToken) {
    return;
  }
  const requestId = createUserFollowRequestId(protestId, fcmToken);
  await firestore.collection('user_follows').doc(requestId).set({
    user: {}, // TODO: Add user
    protestId,
    fcmToken,
    created_at: firebase.firestore.FieldValue.serverTimestamp(),
